...
(function() {
  ...
  // page CSP and offline. Path data from Phosphor icons
  // (https://phosphoricons.com, MIT) matching the original web-font glyphs.
  var ICONS = {
    'plus-circle': '<path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm48-88a8,8,0,0,1-8,8H136v32a8,8,0,0,1-16,0V136H88a8,8,0,0,1,0-16h32V88a8,8,0,0,1,16,0v32h32A8,8,0,0,1,176,128Z"/>',
  ...
  function gridOptions(selected) {
    var labels = { tl:'Top Left',tc:'Top Center',tr:'Top Right',ml:'Middle Left',mc:'Middle Center',mr:'Middle Right',bl:'Bottom Left',bc:'Bottom Center',br:'Bottom Right' };
    var s = '';
    for (var i = 0; i < GRIDS.length; i++) {
      var g = GRIDS[i];
      s += '<option value="' + g + '"' + (selected === g ? ' selected' : '') + '>' + labels[g] + '</option>';
    }
    return s;
  }
  ...
  function moveHL(id, dir) {
    for (var i = 0; i < state.highlights.length; i++) {
      if (state.highlights[i].id === id) {
        var j = i + dir;
        if (j < 0 || j >= state.highlights.length) return;
        var tmp = state.highlights[i];
        state.highlights[i] = state.highlights[j];
        state.highlights[j] = tmp;
        break;
      }
    }
    renumberBadges();
  ...
  function enablePick() {
    ...
    function onMove(e) {
      ...
      if (!el || el === document.body || el === document.documentElement ||
          el.classList.contains('hl-picker-overlay') || el.classList.contains('hl-hover-overlay') || el.classList.contains('hl-overlay')) {
  ...
  function renderHighlights() {
    ...
    for (var i = 0; i < state.highlights.length; i++) {
      var h = state.highlights[i];
      var r = h.rect;
      var px = h.padding, mx = h.margin;

      var finalLeft = r.left - px.left - mx.x;
      var finalTop = r.top - px.top - mx.y;
      var finalW = r.width + px.left + px.right;
      var finalH = r.height + px.top + px.bottom;

      var bgR = parseInt(h.background.color.slice(1,3), 16);
      var bgG = parseInt(h.background.color.slice(3,5), 16);
      var bgB = parseInt(h.background.color.slice(5,7), 16);
      var bgAlpha = h.background.opacity / 100;
      ...
      if (h.badge.visible) {
        ...
        badge.style.cssText = getBadgeStyle(h);
        badge.textContent = h.badge.number;
        wrapper.appendChild(badge);
      }

      if (h.label.visible && h.label.text) {
        var label = document.createElement('div');
        label.style.cssText = getLabelStyle(h);
        label.textContent = h.label.text;
  ...
  function getBadgeStyle(h) {
    var s = h.badge.size || 28;
    var base = 'position:absolute;width:' + s + 'px;height:' + s + 'px;border-radius:50%;' +
      'background:' + h.badge.color + ';color:' + h.badge.textColor + ';' +
      'font:bold ' + Math.max(10, Math.round(s * 0.5)) + 'px/' + s + 'px sans-serif;text-align:center;' +
      'box-shadow:0 2px 6px rgba(0,0,0,0.4);';
    return base + gridPosCSS(h.badge.grid, h.badge.margin);
  }

  function getLabelStyle(h) {
    var fs = h.label.fontSize || 11;
    var base = 'position:absolute;font:bold ' + fs + 'px sans-serif;' +
      'color:' + h.label.color + ';text-shadow:0 1px 3px rgba(0,0,0,0.8);white-space:nowrap;';
  ...
  function gridPosCSS(grid, m) {
    var y = m ? m.y || 0 : 0;
    var x = m ? m.x || 0 : 0;
    function p(v) { return (4 + v) + 'px'; }
    switch (grid) {
      case 'tl': return 'top:' + p(y) + ';left:' + p(x);
      case 'tc': return 'top:' + p(y) + ';left:50%;transform:translateX(calc(-50% + ' + x + 'px))';
      case 'tr': return 'top:' + p(y) + ';right:' + p(-x);
      case 'ml': return 'top:50%;left:' + p(x) + ';transform:translateY(calc(-50% + ' + y + 'px))';
      case 'mc': return 'top:50%;left:50%;transform:translate(calc(-50% + ' + x + 'px), calc(-50% + ' + y + 'px))';
      case 'mr': return 'top:50%;right:' + p(-x) + ';transform:translateY(calc(-50% + ' + y + 'px))';
      case 'bl': return 'bottom:' + p(-y) + ';left:' + p(x);
      case 'bc': return 'bottom:' + p(-y) + ';left:50%;transform:translateX(calc(-50% + ' + x + 'px))';
      case 'br': return 'bottom:' + p(-y) + ';right:' + p(-x);
  ...
  function doCapture(mode, scale) {
    ...
    function cropToHighlights(canvas) {
      ...
      var top = Math.min.apply(null, areas.map(function(a) { return a.top; }));
      var right = Math.max.apply(null, areas.map(function(a) { return a.right; }));
      var bottom = Math.max.apply(null, areas.map(function(a) { return a.bottom; }));
      var cw = right - left, ch = bottom - top;
      if (cw > 0 && ch > 0) {
        var c = document.createElement('canvas');
        c.width = Math.round(cw * scale);
        c.height = Math.round(ch * scale);
        c.getContext('2d').drawImage(canvas, left * scale, top * scale, cw * scale, ch * scale, 0, 0, cw * scale, ch * scale);
        return c;
      }
      return canvas;
    }
